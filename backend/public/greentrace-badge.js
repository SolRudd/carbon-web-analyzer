;(function () {
  ...
  var BASE = me.replace(/\/[^/]+$/, '');
  var API_BASE = BASE; // e.g. https://api.greentracer.org if hosted there
  ...
  // Where result pages live (on your main site)
  ...
  function renderBadge(d, pageUrl, el) {
    ...
    // ✅ Use API-provided slug first, otherwise fallback
    var slug = (d.slug && String(d.slug).trim()) || slugifyFromUrl(pageUrl);

    // ✅ Always point to the correct results page
    ...
    el.innerHTML =
      ...
      'font-size:' +
      fontSize +
      'px;font-weight:800;line-height:1.1;' +
      ...
      '</div>' +
      '</a>' +
      '<div style="margin-top:10px;font-size:16px;color:' +
      t.subText +
      ';text-align:center;">' +
      'Cleaner than ' +
      pct +
