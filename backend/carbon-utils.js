...
async function getDetailedGreenInfo(hostname) {
  try {
    const { data } = await axios.get(`https://api.thegreenwebfoundation.org/greencheck/${hostname}`);
    return {
      isGreen: !!data.green,
...
function calculateAdvancedCarbon(sizeData, greenInfo, region = 'global') {
  ...
  let grams = totalEnergyKWh * ci * (1 - greenInfo.reductionFactor);
  return { carbonGrams: Math.round(grams*100)/100, energyKWh: Math.round(totalEnergyKWh*10000)/10000 };
}
