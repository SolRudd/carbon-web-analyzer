...
// ── CORS Rules ──────────────────────────────────────
// Badge endpoints open to all origins:
const badgeCors = cors({ origin:'*', methods:['GET'] });

// Dynamic CORS for your frontend domains:
...
function dynamicCORS(origin, cb) {
  if (!origin || FRONTEND_ORIGINS.some(r => typeof r === 'string' ? r === origin : r.test(origin))) {
    cb(null, true);
  } else {
...
app.get('/api/check-carbon', limiterCheck, async (req,res) => {
  ...
  try {
    ...
    return res.status(500).json({ error:'Failed to perform carbon check.' });
  }
});

// ── POST /api/check-carbon (manual check) ─────────
...
async function getCached(slug) {
  ...
  return {
    ...
    // if you ever need sizeMB, extract info.sizeInfo.bytes/(1024*1024)
  };
}
...
async function performCarbonCheck(norm, host) {
  ...
  // Upsert only into existing columns + result_data JSON
  const { data: row, error } = await supabase
    .from('results')
    .upsert(
      {
