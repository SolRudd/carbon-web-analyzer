        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-2xl">
                ...
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <Tooltip
                    formatter={(value, _, { payload }) => [
                      ...
                      `Grade ${payload.grade} - ${payload.label}`,
                    ]}
                    contentStyle={{
                      backgroundColor: isDark ? "#1e293b" : "#ffffff",
                      color: tickColor,
                      border: `2px solid ${gradeColors[selectedGrade]}`,
                      borderRadius: "12px",
                  ...
                  <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                    {chartData.map((entry) => (
                      <Cell 
                        ...
                        stroke={selectedGrade === entry.grade ? "#ffffff" : "none"}
            ...
            <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
              {activeChart === "bar" 
                ? "Each bar shows the maximum CO₂e allowed for that grade level"
                : "Smooth curve showing the progression of emissions across grade levels"
              }
            </p>
          </div>
          ...
          <div className="mt-12 bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {chartData.map(({ grade, value, label, count }, index) => (
                    <tr 
                      key={grade} 
                      className={`border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-200 ${
                        selectedGrade === grade ? 'bg-green-50 dark:bg-green-900/20' : ''
                      }`}
                    >
