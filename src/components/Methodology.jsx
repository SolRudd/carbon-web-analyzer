...
import worldWebp from '../assets/world.webp';
import worldAvif from '../assets/world.avif';

export default function Methodology() {
  const steps = [
    {
      icon: <Globe className="w-8 h-8 text-white group-hover:text-green-200 transition-colors" />,
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            {steps.map((step, index) => (
              <motion.div key={index} className="flex items-start gap-6 group" variants={itemVariants}>
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 bg-gradient-to-br from-green-500 to-blue-500 group-hover:from-green-600 group-hover:to-blue-600 rounded-2xl flex items-center justify-center text-white shadow-xl group-hover:shadow-2xl transition-all duration-300 transform group-hover:scale-105">
                    {step.icon}
                  </div>
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="text-base bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-4 py-1 rounded-full font-medium">
                      Step {index + 1}
                    </div>
                  </div>
          ...
          {/* Illustration (now using <picture> with fallbacks) */}
          <motion.div
          ...
          >
            ...
            <div className="relative z-10 bg-white/60 dark:bg-slate-800/60 backdrop-blur-lg rounded-3xl p-8 border border-slate-200 dark:border-slate-700 shadow-2xl flex items-center justify-center max-w-lg mx-auto transform hover:scale-105 transition-transform duration-500">
              {/* ✅ Efficient sources + lazy/async */}
              <picture>
                <source srcSet={worldAvif} type="image/avif" />
                <source srcSet={worldWebp} type="image/webp" />
                <img
                  src={worldPng}
                  ...
                  loading="lazy"
                  decoding="async"
                  // If you know the intrinsic dimensions, uncomment to reduce CLS further:
                  // width="800"
                  // height="800"
                />
              </picture>

              {/* Overlay icons (unchanged) */}
