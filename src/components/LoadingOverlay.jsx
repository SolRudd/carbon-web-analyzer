...
export default function LoadingOverlay() {
  ...
  useEffect(() => {
    const factInterval = setInterval(() => {
      setCurrentFactIndex((prevIndex) => (prevIndex + 1) % ecoFacts.length);
    }, 4000);

    const stageInterval = setInterval(() => {
      setCurrentStage((prevStage) => (prevStage + 1) % analysisStages.length);
    }, 2500);
  ...
  const CurrentStageIcon = analysisStages[currentStage].icon;

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-white/90 dark:bg-slate-950/90 p-4 backdrop-blur-xl"
      aria-live="polite"
      aria-busy="true"
    >
          ...
          <div className="relative p-8 text-center">
            ...
            <div className="relative mb-8 flex justify-center">
              ...
              <div className="relative flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500/20 to-green-600/20 backdrop-blur-sm">
                ...
                {/* Center icon that changes with stage */}
                <motion.div
                  key={currentStage}
                ...
                >
                  <CurrentStageIcon className={`h-6 w-6 ${analysisStages[currentStage].color}`} />
                </motion.div>
              </div>
            </div>
            ...
            <div className="mb-8 space-y-3">
              <motion.h2 
                ...
                className="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent dark:from-white dark:to-slate-300"
              >
            ...
            <div className="relative">
              <div className="flex items-center justify-center mb-4">
                ...
                <span className="px-4 text-sm font-semibold text-emerald-600 dark:text-emerald-400 bg-white/50 rounded-full dark:bg-slate-800/50">
                  ECO INSIGHTS
                </span>
                <div className="h-px bg-gradient-to-r from-transparent via-emerald-300 to-transparent flex-1 dark:via-emerald-700" />
              </div>
