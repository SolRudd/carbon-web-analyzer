...
import globeSvg from "../assets/bubble.svg"; // Your awesome globe SVG
...
export default function InputForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!site) {
      ...
      // Optionally, add a visual cue for the user that the input is empty
    ...
    try {
      ...
      // Wait a minimum of 15 seconds for the "game experience"
  ...
  return (
    ...
    >
      {/* Enhanced background effects - adjusted opacities and sizes for subtlety */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto flex flex-col items-center gap-12"> {/* Increased gap for more breathing room */}
        <motion.div
          className="w-full -mt-20 sm:-mt-16 mb-8 pointer-events-none max-w-[800px]" // Max-width to control globe size on very large screens
          initial="hidden"
          animate="visible"
        ...
        >
          <img
            ...
            className="w-full h-auto pointer-events-none" // Removed hover effect from JS controlled img
          />
        </motion.div>

        <motion.div
          className="text-center space-y-6" // Increased space-y
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          </h2> {/* Stronger, more engaging headline */}
          <p className="text-lg sm:text-xl font-medium text-slate-700 dark:text-slate-300 max-w-lg mx-auto leading-relaxed"> {/* Larger text */}
        ...
        <motion.form
          ...
          className="w-full grid grid-cols-1 sm:grid-cols-3 gap-6 bg-white/70 dark:bg-white/10 backdrop-blur-md border border-slate-300 dark:border-white/20 p-8 rounded-3xl mx-auto max-w-3xl shadow-xl hover:shadow-2xl transition-all duration-300" // Increased padding, rounded corners, gap
          initial="hidden"
          animate="visible"
        ...
        >
          ...
          <p className="col-span-1 sm:col-span-3 text-center text-sm text-slate-600 dark:text-slate-400 mt-4 opacity-85"> {/* Slightly larger legal text */}
        ...
        {loading && <LoadingOverlay />} {/* Still show the full overlay for the "game experience" */}
      </div>
    </section>
  );
}