...
import {
  Coffee, Zap, Smartphone, Leaf, ChevronLeft, ChevronRight, Rocket,
} from "lucide-react";
...
export default function ImpactStats({ carbonPerView = 0, siteUrl }) {
  ...
  const G_PER_TREE_YEAR = 21770;

  const equivalencies = [
    {
      icon: <Coffee className="w-8 h-8" />,
      value: Math.max(0, Math.round(annualCO2gRaw / TEA_G_PER_CUP)).toLocaleString(),
    ...
    },
    {
      ...
      value: (annualCO2gRaw / G_PER_KWH).toFixed(1),
    ...
    },
    {
      ...
      value: Math.max(0, Math.round(annualCO2gRaw / G_PER_CHARGE)).toLocaleString(),
    ...
    },
    {
      ...
      value: (annualCO2gRaw / G_PER_TREE_YEAR).toFixed(2),
  ...
  return (
    <section className="relative overflow-hidden bg-slate-50 dark:bg-slate-900/50 py-5 px-4 sm:px-6 lg:px-8">
      ...
      <div className="absolute inset-0 opacity-10 dark:opacity-20 pointer-events-none">
        <motion.div initial={{ scale: 0.9, rotate: 10, x: '50%', y: '-50%' }} animate={{ scale: 1.3, rotate: -5, x: '40%', y: '-40%' }} transition={{ duration: 22, repeat: Infinity, ease: "linear", repeatType: "mirror" }} className="absolute top-1/4 right-0 w-[50rem] h-[50rem] bg-green-400/10 dark:bg-green-400/15 rounded-full blur-3xl" />
        <motion.div initial={{ scale: 0.9, rotate: -10, x: '-50%', y: '50%' }} animate={{ scale: 1.2, rotate: 5, x: '-60%', y: '40%' }} transition={{ duration: 25, repeat: Infinity, ease: "linear", repeatType: "mirror" }} className="absolute bottom-1/4 left-0 w-[40rem] h-[40rem] bg-blue-400/10 dark:bg-blue-400/15 rounded-full blur-3xl" />
      </div>

      <motion.div className="relative z-10 max-w-5xl mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={containerVariants}>
        {/* Header */}
        <motion.div className="text-center mb-12" variants={itemVariants}>
          ...
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-300 max-w-3xl mx-auto">
            Change the traffic to see the annual impact from your per-view emissions.
          </p>
        </motion.div>

        {/* Card */}
        <motion.div className="bg-white/60 dark:bg-slate-800/20 backdrop-blur-xl border border-slate-200 dark:border-slate-700 rounded-3xl p-8 sm:p-10 shadow-2xl mb-16" variants={itemVariants}>
          {/* Selector */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 bg-slate-100/50 dark:bg-slate-900/30 p-6 rounded-2xl mb-8">
            ...
            <button onClick={prev} disabled={idx === 0} aria-label="Decrease monthly views" className="p-3 rounded-full bg-white dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow">
            ...
            <button onClick={next} disabled={idx === VIEW_OPTIONS.length - 1} aria-label="Increase monthly views" className="p-3 rounded-full bg-white dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow">
          ...
          <div className="text-center space-y-2">
            <p className="text-lg text-slate-700 dark:text-slate-300">
              <span className="font-semibold">{monthlyViews.toLocaleString()} views/month × 12 months</span>
              {siteUrl && (
                <> — annual footprint for <span className="font-semibold">{new URL(siteUrl).hostname}</span></>
              )}
            </p>
        ...
        <motion.div className="text-center mb-8" variants={itemVariants}>
          ...
          <p className="text-lg text-slate-600 dark:text-slate-300">
            (Each number below reflects {monthlyViews.toLocaleString()} views/month over a year)
          </p>
        </motion.div>

        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" variants={containerVariants}>
          {equivalencies.map((item, i) => (
            <motion.div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-6 text-center shadow-lg hover:shadow-xl border border-slate-200 dark:border-slate-700 transform hover:-translate-y-2 transition-all duration-300" variants={itemVariants}>
        ...
        <motion.div className="mt-24" variants={itemVariants}>
          <div className="bg-gradient-to-r from-green-500/10 to-blue-500/10 border border-green-500/20 dark:border-green-400/20 rounded-3xl p-10 text-center">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300 mb-8 max-w-2xl mx-auto">
              Small performance wins compound into lower energy and carbon per visit.
            </p>
            <a
              href="https://buzzboost.co.uk/contact" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-10 py-5 bg-gradient-to-r from-green-500 to-blue-500 hover:from-green-600 hover:to-blue-600 text-white font-bold rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg"
            >
